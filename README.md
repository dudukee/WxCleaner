...
### 方式一：运行可执行文件
在 [Releases](https://github.com/yourusername/WxCleaner/releases) 页面下载对应系统的安装包或可执行文件，双击即可运行。
...
1.  **选择路径**: 点击 "浏览" 按钮，选择您的微信文件存储目录。Windows 通常位于 `Documents/WeChat Files`；macOS 通常位于 `~/Library/Containers/com.tencent.xinWeChat/Data/Documents` 或微信设置中配置的文件目录。
...
# macOS
pyinstaller --windowed --name WxCleaner main.py
```
